Scaling [tml-server-v1-plugin-aefa-ml_agenticai_restapi] With Kubernetes
===========================
...
Based on your TML solution [tml-server-v1-plugin-aefa-ml_agenticai_restapi] - if you want to scale your application with Kubernetes - you will need to apply the following YAML files.

.. list-table::

   * - **YML File**
     - **Description**
   * - :ref:`tml-server-v1-plugin-aefa-ml_agenticai_restapi.yml`
   ...
   * - :ref:`qdrant.yml`
     - This is the Qdrant deployment YAML.
       ...
       applied if using Step 9 DAG.
   * - :ref:`nginx-ingress-tml-server-v1-plugin-aefa-ml_agenticai_restapi.yml`
     - If you are scaling your TML solution you must

       apply the nginx-ingresstml-server-v1-plugin-aefa-ml_agenticai_restapi.yml; this yaml is 
...
.. code-block:: YAML
   ...
   kubectl apply -f kafka.yml -f secrets.yml -f mysql-storage.yml -f mysql-db-deployment.yml -f qdrant.yml -f privategpt.yml -f ollama.yml -f tml-server-v1-plugin-aefa-ml_agenticai_restapi.yml

tml-server-v1-plugin-aefa-ml_agenticai_restapi.yml
------------------------

.. important::
   Copy and Paste this YAML file: tml-server-v1-plugin-aefa-ml_agenticai_restapi.yml - and save it locally.

   These YML files can also be found on GitHub at: https://github.com/tsstmldemo/tsstmldemo/tree/main/tml-airflow/dags/tml-solutions/tml-server-v1-plugin-aefa/ymls/tml-server-v1-plugin-aefa-ml_agenticai_restapi
...
.. code-block:: YAML
   ...
   ################# tml-server-v1-plugin-aefa-ml_agenticai_restapi.yml
     ...
     kind: Deployment
     metadata:
       name: tml-server-v1-plugin-aefa-ml_agenticai_restapi
     spec:
       selector:
         matchLabels:
           app: tml-server-v1-plugin-aefa-ml_agenticai_restapi
       ...
       template:
         ...
         spec:
           containers:
           - name: tml-server-v1-plugin-aefa-ml_agenticai_restapi
             image: maadsdocker/tml-server-v1-plugin-3f10-ml_agenticai_restapi-amd64:latest
             volumeMounts:
             - name: dockerpath
             ...
             - name: TSS
               value: '0'
             - name: PROJECTNAME
               value: 'tml-server-v1-plugin-aefa'               
             - name: SOLUTIONNAME
               value: 'tml-server-v1-plugin-aefa-ml_agenticai_restapi'
             - name: SOLUTIONDAG
               value: 'solution_preprocessing_ml_agenticai_restapi_dag-tml-server-v1-plugin-aefa'
             - name: GITUSERNAME
               value: 'tsstmldemo'
             - name: GITREPOURL
   ...
   ---
     ...
     kind: Service
     metadata:
       name: tml-server-v1-plugin-aefa-ml_agenticai_restapi-visualization-service
       labels:
         app: tml-server-v1-plugin-aefa-ml_agenticai_restapi-visualization-service
     spec:
       type: ClusterIP
       ports:
   ...
   ---
     ...
     kind: Service
     metadata:
       name: tml-server-v1-plugin-aefa-ml_agenticai_restapi-external-service
       labels:
         app: tml-server-v1-plugin-aefa-ml_agenticai_restapi-external-service
     spec:
       type: ClusterIP
       ports:
       - port: 80 # Ingress port, if using port 443 will need to setup TLS certs
      ...
      spec:
        ...
        template:
          ...
          spec:
            containers:
            - name: privategpt
              image:  # IF you DO NOT have NVIDIA GPU use: maadsdocker/tml-privategpt-no-gpu-amd64
...
.. code-block::
   ...
   kubectl port-forward deployment/tml-server-v1-plugin-aefa-ml_agenticai_restapi 6060:6060
...
.. important::
   ...
   **STEP 4:  Apply nginx-ingress-tml-server-v1-plugin-aefa-ml_agenticai_restapi.yml to your kubernetes cluster.  First you need to save it locally then apply it:**

nginx-ingress-tml-server-v1-plugin-aefa-ml_agenticai_restapi.yml
-------------
   ...
   .. code-block::
      ...
      kubectl apply -f nginx-ingress-tml-server-v1-plugin-aefa-ml_agenticai_restapi.yml
